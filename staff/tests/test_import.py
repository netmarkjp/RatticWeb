...
class ImportTests(TestCase):
    ...
    def test_upload_keepass(self):
        ...
        resp = self.data.staff.get(reverse('staff:upload_keepass'))
        ...
        with open('docs/keepass/test2.kdb') as fp:
            post['file'] = fp
            resp = self.data.staff.post(reverse('staff:upload_keepass'), post, follow=True)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_process_import_no_data(self):
        ...
        resp = self.data.staff.get(reverse('staff:import_overview'))
    ...
    def test_process_import_last_entry(self):
        ...
        self.assertRedirects(resp, reverse('staff:home'), 302, 200)
    ...
    def test_process_import_entry_ignore(self):
        ...
        resp = self.data.staff.get(reverse('staff:import_ignore', args=(0, )))

        # Check things worked
        self.assertRedirects(resp, reverse('staff:import_overview'), 302, 302)
        self.assertNotIn('imported_data', self.data.staff.session)
    ...
    def test_process_import_entry_import(self):
        ...
        # Load the import screen
        resp = self.data.staff.get(reverse('staff:import_process', args=(0, )))
        ...
        resp = self.data.staff.post(
            reverse('staff:import_process', args=(0, )),
            post,
        )
